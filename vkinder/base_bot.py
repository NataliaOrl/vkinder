...
def get_params_search(user_id, text): 
        if text in ('привет', 'здравствуй', 'hello', 'hi', 'заново', 'restart') :               
            write_msg(user_id, f'Добро пожаловать в самое лучшее приложение для знакомст!\n'
                                f'Введите минимальный возраст будущего знакомого')
            user = user_id           
            msg = bot_speak()[1]
            if msg.isdigit():
                min_age = int(msg)
